...
每个视频默认输出 MP4、JSON（元数据 + 原始机器转写 + 分段时间戳）和 TXT 到同一个结果目录。作为 Agent Skill 使用时，Agent 必须完整审阅已有 TXT，只原地修正上下文可明确确认的错别字、同音字、术语、标点和断句；不润色、不扩写、不总结、不改变原意，不确定内容原样保留。JSON 不同步修改，也不生成多版本 TXT。程序的 `completed` 只代表机器阶段完成；请求转写时，Agent 完成全部 TXT 审阅才是 Skill 的最终完成条件。转写失败时没有 TXT 审阅步骤，任务报告为未完成。

`.temp` 保存最终 MP4 的可复用缓存，用于断点续传和补处理；传 `--no-video-output` 时，MP4 只保留在 `.temp`，不复制到结果目录。WAV 和 DASH 分轨文件属于中间文件，处理完成后清理。
