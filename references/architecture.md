...
每个视频默认输出 MP4、JSON（元数据 + 文案 + 分段时间戳）和 TXT（纯文本转写）到同一个结果目录。`.temp` 保存最终 MP4 的可复用缓存，用于断点续传和补处理；传 `--no-video-output` 时，MP4 只保留在 `.temp`，不复制到结果目录。WAV 和 DASH 分轨文件属于中间文件，处理完成后清理。
